...
object MarkdownParser {
    ...
    fun clear(string: String?): String? {
        string ?: return null
        val mt = parse(string)
        val element = Element.Text("", mutableListOf())
    ...
    private fun findElements(string: CharSequence): List<Element> {
        ...
        loop@ while (matcher.find(lastStartIndex)) {
            ...
            when (group) {
                ...
                //HEADER
                2 -> {
                    val reg = "^#{1,6}".toRegex().find(string.subSequence(startIndex, string.length))
